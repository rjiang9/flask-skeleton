...
def update_spec(id):
    ...
    return render_template('spec/specs.html', title='Edit Spec', form=form, spec=spec_obj, check=check)
...
@spec_bp.route('/add-spec', methods=['GET', 'POST'])
@login_required
def specs():
    check= None
    user = current_user
...
def delete_spec(id):
    ...
    print('delete spec with id={} by {}'.format(id, user))

    # spec = Spec.query.filter_by(id=int(id), author=user).one()
    spec = Spec.query.filter_by(id=int(id)).one()
